...
- [x] ✅ **كوبونات وعروض — مكتمل يوليو 8, 2026** (راجع STATUS.md §73): جدول `coupons` (migration 068) + `modules/coupons/` backend كامل (CRUD + validate/redeem ذرّي عبر RPC) + دمج بـ`InvoicesService.create()` + صفحة `/dashboard/coupons` frontend كاملة (owner/manager). **متبقٍ**: لا حقل إدخال كود كوبون بشاشة الدفع بالـPOS بعد (`PaymentModal.tsx`) — الـbackend جاهز، الكاشير لا يقدر يدخل الكود من واجهة البيع نفسها بعد. **لم يُدفَع/يُنشَر بعد** — بانتظار مراجعة المستخدم
