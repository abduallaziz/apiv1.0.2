...
- [ ] COGS (تكلفة المنتج) — **لم يُبنَ عمدًا بعد اكتشاف فجوة أكبر أثناء محاولة بنائه** (راجع الملاحظة الحرجة أسفل هذا القسم) — بناء تقرير COGS بلا مصدر حقيقي لتكلفة المبيعات سيكون تقريرًا مضلِّلًا (يرجع صفر دائمًا)
...
> ⚠️ **اكتشاف حرج أثناء محاولة بناء تقرير COGS (July 3, 2026) — يحتاج قرار من المستخدم قبل أي تنفيذ**:
> **مسار البيع بالـPOS (`InvoicesService.create()`) لا يلمس المخزون إطلاقًا.** لا `stock_movements` يُنشَأ عند البيع (قيمة enum `'sale'` موجودة بالـCHECK constraint منذ البداية لكن لا كود ينشئها أبدًا)، لا خصم من `stock_levels.quantity_on_hand`، لا استهلاك من `cost_layers`. بعبارة أخرى: **"Inventory Core" الموسوم ✅ مكتمل بالكامل بأعلى هذا الملف، وموديول POS/الفواتير، منفصلان تمامًا اليوم** — بيع أي منتج عبر نقطة البيع لا يقلّل مخزونه إطلاقًا مهما بيع.
> السبب في عدم إصلاحها فورًا تلقائيًا: الأدوات الذرّية الجاهزة فعليًا موجودة (`fn_apply_stock_movement`، `fn_consume_cost_layers` من migration 019) — الوصل تقني بسيط نظريًا — **لكن** لا يوجد ربط مضمون بين `orders.branch_id` و`warehouses.id` (العمود `warehouses.branch_id` **اختياري** — قد يكون فرع بلا مستودع، أو مستودع بلا فرع، أو عدة مستودعات لفرع واحد). ربط هذا تلقائيًا بافتراض خاطئ قد يخصم من مستودع خطأ لعدد غير معروف من المستأجرين، على **أخطر مسار بالنظام كله** (كل عملية بيع). هذا قرار معماري (كيف يُحدَّد المستودع الصحيح لكل فرع/عملية بيع؟ هل كل عنصر مُتتبَّع بالمخزون إلزاميًا أم اختياري حسب نوع النشاط؟) يحتاج قرار صريح من المستخدم قبل التنفيذ، وليس شيئًا يُفترَض تلقائيًا أثناء عمل مستقل بدون إشراف.
