...
- الإصلاح الرابع (`customer_id` عمومًا بالفاتورة) لم يُختبر حيًّا بعد وقت كتابة هذا القسم (أُضيف بعد جولة الاختبار الأولى) — `tsc`+`build` نظيفان فقط حتى الآن.

## الحالة النهائية
الميزتان (Loyalty Tiers، بطاقات الهدايا) مدفوعتان ومنشورتان على production. إصلاحات الثغرات 1-3 مدفوعة ومنشورة ومُتحقَّق منها حيًّا. الإصلاح الرابع جاهز محليًا (type-check + build نظيفان) بانتظار الدفع.
