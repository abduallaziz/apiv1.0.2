...
export class PlatformAnalyticsRepository {
  ...
  private getDateRange(period: AnalyticsPeriod): { from: Date; to: Date } {
    ...
    switch (period) {
      ...
      case AnalyticsPeriod.LAST_6_MONTHS:
        ...
        break;
      case AnalyticsPeriod.LAST_12_MONTHS:
  ...
  async getMRR(): Promise<number> {
    ...
    return (data as any[]).reduce(
      (sum: number, row: any) => sum + (row.plans?.price_monthly ?? 0),
      0,
    );
  }
  ...
  async getMRRHistory(
    ...
    const { data } = await this.supabase
      .from('subscriptions')
      .select(
        'started_at, ends_at, cancelled_at, status, plan_id, plans!inner(price_monthly)',
      )
  ...
  async getChurnRate(
    ...
    const [{ data: churned }, { count: startingCount }] = await Promise.all([
      ...
      this.supabase
        ...
        .select('*', { count: 'exact', head: true })
        .eq('status', 'active')
        .lte('started_at', range.from.toISOString()),
    ]);
    ...
    const monthly = Array.from(monthlyMap.entries())
      ...
      .map(([month, c]) => ({
        month,
        churned: c,
        rate: Math.round((c / start) * 10000) / 100,
      }));
  ...
  async getGrowthRate(
    ...
    if (!data || data.length === 0)
      return { growthRate: 0, newTenants: 0, monthly: [] };
    const monthlyMap = new Map<string, number>();
    ...
    const monthly = Array.from(monthlyMap.entries())
      ...
      .map(([month, newCount]) => {
        cumulative += newCount;
        return { month, new: newCount, cumulative };
      });
    const growthRate =
      startVal === 0
        ? 100
        : Math.round(((cumulative - startVal) / startVal) * 10000) / 100;
  ...
  async getConversionFunnel(
    ...
    const range = this.buildRange(period, customFrom, customTo);
    const { data: signupData } = await this.supabase
      .from('tenants')
    ...
    const [{ data: trialData }, { data: paidData }, { data: churnedData }] =
      await Promise.all([
        this.supabase
          .from('subscriptions')
          .select('tenant_id')
        ...
        this.supabase
          .from('subscriptions')
          .select('tenant_id, started_at, plans!inner(price_monthly)')
          .in('tenant_id', tenantIds)
          .eq('status', 'active')
          .gt('plans.price_monthly', 0),
        this.supabase
          .from('subscriptions')
          .select('tenant_id, started_at, cancelled_at')
          .in('tenant_id', tenantIds)
          .in('status', ['cancelled', 'expired'])
          .not('cancelled_at', 'is', null),
      ]);
    const activatedTrial = new Set(
      (trialData ?? []).map((s: any) => s.tenant_id),
    ).size;
    const convertedToPaid = new Set(
      (paidData ?? []).map((s: any) => s.tenant_id),
    ).size;
    const churned = new Set((churnedData ?? []).map((s: any) => s.tenant_id))
      .size;
    let avgDaysToConvert = 0;
    if (paidData && paidData.length > 0) {
      const signupMap = new Map(
        (signupData ?? []).map((t: any) => [t.id, t.created_at]),
      );
    ...
    return {
      ...
      trialConversionRate:
        activatedTrial === 0
          ? 0
          : Math.round((convertedToPaid / activatedTrial) * 10000) / 100,
      overallConversionRate:
        signups === 0
          ? 0
          : Math.round((convertedToPaid / signups) * 10000) / 100,
      avgDaysToConvert,
    };
  }
  ...
  > {
    const [{ data: tenants }, { data: subs }] = await Promise.all([
      ...
      this.supabase
        .from('subscriptions')
        .select('tenant_id, status, started_at, ends_at, cancelled_at')
        .in('status', ['active', 'cancelled', 'expired']),
    ]);
    ...
    return Array.from(cohortMap.entries())
      ...
      .map(([cohortMonth, tenantIds]) => {
        ...
        const maxOffset = Math.floor(
          (now.getTime() - cohortStart.getTime()) / (86400000 * 30),
        );
        const retentionByMonth = Array.from(
          { length: Math.min(maxOffset + 1, 12) },
          (_, offset) => {
            ...
            const retained = tenantIds.filter((tid) => {
              ...
              return tenantSubs.some((s: any) => {
                ...
                const end = s.cancelled_at
                  ? new Date(s.cancelled_at)
                  : s.ends_at
                    ? new Date(s.ends_at)
                    : now;
            ...
            return {
              monthOffset: offset,
              retained,
              rate: Math.round((retained / tenantIds.length) * 10000) / 100,
            };
          },
        );
  ...
  async getRevenueByPlan(): Promise<
    { planName: string; tenantCount: number; mrr: number; percentage: number }[]
  > {
    const { data } = await this.supabase
      .from('subscriptions')
    ...
    for (const row of data as any[]) {
      if (!planMap.has(row.plan_id)) {
        planMap.set(row.plan_id, {
          name: row.plans.name,
          price: row.plans.price_monthly,
          count: 0,
        });
      }
      planMap.get(row.plan_id).count++;
    }
    const totalMrr = Array.from(planMap.values()).reduce(
      (s: number, p: PlanEntry) => s + p.price * p.count,
      0,
    );
    return Array.from(planMap.values()).map((plan: PlanEntry) => ({
      ...
      percentage:
        totalMrr === 0
          ? 0
          : Math.round(((plan.price * plan.count) / totalMrr) * 10000) / 100,
    }));
  }
