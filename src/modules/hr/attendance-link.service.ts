...
export class AttendanceLinkService {
  ...
  async check(
    ...
    if (
      user.attendance_device_fingerprint &&
      user.attendance_device_fingerprint !== deviceFingerprint
    ) {
    ...
    if (!user.attendance_device_fingerprint) {
      ...
      this.logAudit({
        ...
        actorId: user.id,
        action: 'attendance.link.device.bound',
      });
    }

    const geo = await this.geofence.checkLocation(
      user.tenant_id,
      user.id,
      lat,
      lng,
    ...
    if (!geo.allowed) {
      throw new BadRequestException(
        'You must be at your work location to check in or out.',
      );
    }

    const code = randomBytes(3).toString('hex').toUpperCase();
    const open = await this.attendanceRepo.findOpenRecord(
      user.tenant_id,
      user.id,
    );

    if (open) {
      await this.attendanceRepo.checkOut(user.tenant_id, open.id, {
        lat,
        lng,
        code,
      });
    ...
    const created = await this.attendanceRepo.checkIn(
      user.tenant_id,
      user.id,
      null,
      { lat, lng, code },
  ...
  async getDashboard(token: string) {
    ...
    const [
      ...
      this.attendanceRepo.findTodayRecord(user.tenant_id, user.id),
      this.attendanceRepo.findForRange(
        user.tenant_id,
        user.id,
        monthStart,
        monthEnd,
    ...
    const workDaysThisMonth = new Set(
      monthRecords.map((r) => r.check_in_at.substring(0, 10)),
    ).size;
    const monthlyHoursTotal = monthRecords.reduce((sum, r) => {
      ...
      return (
        sum + (now.getTime() - new Date(r.check_in_at).getTime()) / 3600000
      );
    }, 0);
    ...
    return {
      ...
      leave_balance:
        user.annual_leave_balance !== null &&
        user.annual_leave_balance !== undefined
  ...
  async getEmployeeLeaveBalance(
    ...
    if (annualLeaveBalance === null || annualLeaveBalance === undefined)
      return null;
    const usedThisYear = await this.leaveRequestsRepo.sumApprovedDaysThisYear(
      tenantId,
      userId,
    );
  ...
  private async validateLeaveBalanceBeforeCreation(
    ...
    const available = await this.getEmployeeLeaveBalance(
      tenantId,
      userId,
      annualLeaveBalance,
    );
  ...
  async createLeaveRequest(
    ...
    await this.validateLeaveBalanceBeforeCreation(
      user.tenant_id,
      user.id,
    ...
    const created = await this.leaveRequestsRepo.create(
      user.tenant_id,
      user.id,
      dto,
    );
