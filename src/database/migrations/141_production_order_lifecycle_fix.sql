-- Migration 6.8 — Production Order lifecycle fix.
--
-- fn_post_production_order (112) originally only accepted status='draft'.
-- Migration 6.6 added a start() action (draft -> in_progress) at the
-- application layer, but never touched this RPC — so start()+complete()
-- always failed with "is not draft", confirmed by a regression test in
-- Migration 6.7. This widens the guard to accept 'in_progress' too. All
-- consumption/costing/stock-movement logic below is byte-for-byte
-- unchanged from 112/140 — only the status guard condition changes.
...
BEGIN
  ...
  IF v_order.status NOT IN ('draft', 'in_progress') THEN
    RAISE EXCEPTION 'production order % is not draft or in_progress (status=%)', p_production_order_id, v_order.status;
  END IF;
