...
describe('inventory snapshots regression (Migration 11.1b)', () => {
  ...
  beforeAll(async () => {
    ...
    const { data: wh2, error: wh2Err } = await supabase
      ...
      .insert({
        ...
        name: `SNAP11 WH2 ${Date.now()}`,
        code: `SNAP2${Date.now() % 100000}`,
      })
      .select()
      .single();
  ...
  it('Test 1: snapshot creation — one run created, correct item rows and quantities', async () => {
    ...
    const { data: run, error } = await supabase.rpc(
      'fn_generate_inventory_snapshot',
      {
  ...
  it('Test 2: valuation correctness — matches fn_inventory_valuation_report exactly', async () => {
    ...
    const { data: valuationReport, error: valErr } = await supabase.rpc(
    ...
    expect(Number(snapItems!.quantity_on_hand)).toBe(
      Number(valRow.quantity_on_hand),
    );
    expect(Number(snapItems!.average_unit_cost)).toBe(
      Number(valRow.average_unit_cost),
    );
  ...
  it('Test 3: multiple warehouses/items captured in one snapshot', async () => {
    ...
    const itemCWh1 = items.find(
      (i: any) => i.item_id === itemC && i.warehouse_id === warehouseId,
    );
    const itemCWh2 = items.find(
      (i: any) => i.item_id === itemC && i.warehouse_id === warehouseId2,
    );
    const itemDWh1 = items.find(
      (i: any) => i.item_id === itemD && i.warehouse_id === warehouseId,
    );
  ...
  it('Test 5: immutability — duplicate generation rejected, supersede creates a new active run, old run retained unchanged', async () => {
    ...
    const { data: firstRun, error: firstErr } = await supabase.rpc(
      'fn_generate_inventory_snapshot',
      {
        p_tenant_id: TEST_TENANT_ID,
        p_actor_id: null,
    ...
    const { error: dupErr } = await supabase.rpc(
      'fn_generate_inventory_snapshot',
      {
        p_tenant_id: TEST_TENANT_ID,
        p_actor_id: null,
    ...
    const { data: secondRun, error: secondErr } = await supabase.rpc(
      'fn_generate_inventory_snapshot',
      {
        p_tenant_id: TEST_TENANT_ID,
        p_actor_id: null,
    ...
    const { data: firstRunAfter } = await supabase
      .from('inventory_snapshot_runs')
      .select('status')
      .eq('id', firstRun.id)
      .single();
