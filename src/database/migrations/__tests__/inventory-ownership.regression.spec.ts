...
describe('inventory ownership regression (Migration 10.1)', () => {
  ...
  const createDraftReceipt = async (itemId: string, qty: number, unitCost: number, ownership?: { ownership_type: string; owner_supplier_id?: string }) => {
  ...
  beforeAll(async () => {
    ...
    const { data: supplier, error: supErr } = await supabase
      .from('suppliers')
      .insert({ tenant_id: TEST_TENANT_ID, name: `OWN10 Supplier ${Date.now()}` })
      .select()
      .single();
  ...
  afterAll(async () => {
    for (const id of layerIds) await supabase.from('stock_ownership_layers').delete().eq('id', id);
  ...
  it('company-owned receipt (default, no ownership fields): no layer created, posting identical to plain fn_post_goods_receipt', async () => {
    ...
    const { data: posted, error } = await supabase.rpc('fn_post_goods_receipt_with_ownership', {
  ...
  it('consignment receipt: creates an ownership layer with correct supplier/quantity, and stock/cost posting is unaffected', async () => {
    ...
    const grId = await createDraftReceipt(itemId, 30, 7, { ownership_type: 'consignment', owner_supplier_id: supplierId });
  ...
  it('duplicate posting: second call is rejected by the inherited fn_post_goods_receipt guard, and no duplicate layer is created', async () => {
    ...
    const grId = await createDraftReceipt(itemId, 10, 5, { ownership_type: 'consignment', owner_supplier_id: supplierId });

    const first = await supabase.rpc('fn_post_goods_receipt_with_ownership', { p_goods_receipt_id: grId, p_actor_id: null });
    expect(first.error).toBeNull();

    const second = await supabase.rpc('fn_post_goods_receipt_with_ownership', { p_goods_receipt_id: grId, p_actor_id: null });
  ...
  it('ownership transfer: closes the old layer and creates a new active one with the new owner', async () => {
    ...
    const { data: oldLayer } = await supabase.from('stock_ownership_layers').select('status').eq('id', layer.id).single();
  ...
  it('fn_consume_ownership_layers: partial then full consumption closes the layer; returns 0 when no layer exists', async () => {
    ...
    const { data: consumed1, error: err1 } = await supabase.rpc('fn_consume_ownership_layers', {
    ...
    const { data: afterPartial } = await supabase.from('stock_ownership_layers').select('quantity, status').eq('id', layer.id).single();
    expect(Number(afterPartial!.quantity)).toBe(15);
    expect(afterPartial!.status).toBe('active');

    const { data: consumed2, error: err2 } = await supabase.rpc('fn_consume_ownership_layers', {
      p_tenant_id: TEST_TENANT_ID,
      p_warehouse_id: warehouseId,
      p_item_id: ownedItem,
      p_variant_id: null,
    ...
    const { data: afterFull } = await supabase.from('stock_ownership_layers').select('quantity, status').eq('id', layer.id).single();
    ...
    const { data: consumedNone, error: errNone } = await supabase.rpc('fn_consume_ownership_layers', {
      p_tenant_id: TEST_TENANT_ID,
      p_warehouse_id: warehouseId,
      p_item_id: plainItem,
      p_variant_id: null,
