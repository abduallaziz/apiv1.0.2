...
describe('Advanced Warehouse Management (migration 13.20)', () => {
  ...
  beforeAll(async () => {
    ...
    const { data: locR } = await supabase
      .from('warehouse_locations')
      .insert({
        tenant_id: TENANT,
        warehouse_id: warehouseId,
        code: `W1320-R-${runSuffix}`,
        name: 'Receiving Test',
        location_purpose: 'receiving',
      })
      .select()
      .single();
    ...
    const { data: locS } = await supabase
      ...
      .insert({
        tenant_id: TENANT,
        warehouse_id: warehouseId,
        code: `W1320-S-${runSuffix}`,
        name: 'Storage Test',
        location_purpose: 'storage',
        max_quantity: 1000,
      })
      .select()
      .single();
  ...
  afterAll(async () => {
    ...
    await supabase
      ...
      .delete()
      .in('location_id', cleanupLocationIds);
    await supabase
      .from('warehouse_locations')
      .update({ is_active: false, deleted_at: new Date().toISOString() })
      .in('id', cleanupLocationIds);
    await supabase
      .from('items')
      .update({ is_active: false, deleted_at: new Date().toISOString() })
  ...
  it('1. location hierarchy unchanged: existing zone/aisle/rack/shelf/bin validation trigger still enforces order', async () => {
    ...
    const bad = await supabase.from('warehouse_locations').insert({
      tenant_id: TENANT,
      warehouse_id: warehouseId,
      code: `W1320-BAD-${suffix}`,
      name: 'Bad Rack',
      location_type: 'rack',
  ...
  it('2. location purpose assignment (independent of structural type)', async () => {
    const { data, error } = await supabase
      .from('warehouse_locations')
      .select('location_purpose, max_quantity')
      .eq('id', locStorage)
      .single();
  ...
  it('3. capacity validation: fn_suggest_putaway_location reports capacity_ok=false when it would exceed max_quantity', async () => {
    const { data: rule } = await supabase
      ...
      .insert({
        tenant_id: TENANT,
        warehouse_id: warehouseId,
        name: 'WMS 13.20 Capacity Rule',
        applies_to_item_id: itemId,
        target_location_id: locStorage,
      })
      .select()
      .single();
    const sugg = await supabase.rpc('fn_suggest_putaway_location', {
      p_tenant_id: TENANT,
      p_warehouse_id: warehouseId,
      p_item_id: itemId,
      p_category_id: null,
    ...
    const sugg2 = await supabase.rpc('fn_suggest_putaway_location', {
      p_tenant_id: TENANT,
      p_warehouse_id: warehouseId,
      p_item_id: itemId,
      p_category_id: null,
  ...
  it('5. putaway task creation + suggestion generation', async () => {
    const task = await supabase.rpc('fn_create_putaway_task', {
      ...
      p_batch_id: null,
      p_quantity: 30,
      p_source_location_id: locReceiving,
      p_suggested_location_id: locStorage,
  ...
  it('6. putaway task completion moves stock between locations (real, not fake)', async () => {
    ...
    await supabase.rpc('fn_assign_warehouse_task', {
      p_task_id: task.data.id,
      p_tenant_id: TENANT,
      p_assigned_to: null,
      p_actor_id: null,
    });
    const confirm = await supabase.rpc('fn_confirm_warehouse_task', {
      p_task_id: task.data.id,
      p_tenant_id: TENANT,
      p_quantity: 20,
      p_confirmed_location_id: locStorage,
      p_actor_id: null,
    });
    expect(confirm.error).toBeNull();
    expect(confirm.data.status).toBe('completed');

    const balStorage = await supabase
      .from('stock_levels')
      .select('quantity_on_hand')
      .eq('tenant_id', TENANT)
      .eq('item_id', itemId)
      .eq('location_id', locStorage)
      .maybeSingle();
  ...
  it('7. replenishment low-stock trigger creates a task', async () => {
    const { data: rule } = await supabase
      ...
      .insert({
        ...
        warehouse_id: warehouseId,
        item_id: itemId,
        destination_location_id: locReceiving,
        source_location_id: locStorage,
        min_quantity: 500,
        max_quantity: 600,
      })
      .select()
      .single();
    const tasks = await supabase.rpc('fn_run_replenishment_check', {
      p_tenant_id: TENANT,
      p_warehouse_id: warehouseId,
      p_created_by: null,
    });
    expect(tasks.error).toBeNull();
    const created = (tasks.data as any[]).find(
      (t) => t.source_document_id === rule.id,
    );
  ...
  it('8. replenishment task completion moves stock; refuses without a source location', async () => {
    ...
    const task = await supabase.rpc('fn_create_putaway_task', {
      ...
      p_batch_id: null,
      p_quantity: 5,
      p_source_location_id: locStorage,
      p_suggested_location_id: locReceiving,
      p_source_document_type: 'manual',
      p_source_document_id: null,
      p_created_by: null,
    });
    await supabase
      .from('warehouse_tasks')
      .update({ task_type: 'replenishment' })
      .eq('id', task.data.id);
    await supabase.rpc('fn_assign_warehouse_task', {
      p_task_id: task.data.id,
      p_tenant_id: TENANT,
    ...
    const confirm = await supabase.rpc('fn_confirm_warehouse_task', {
      ...
      p_tenant_id: TENANT,
      p_quantity: 5,
      p_confirmed_location_id: locReceiving,
      p_actor_id: null,
    });
    expect(confirm.error).toBeNull();
    expect(confirm.data.status).toBe('completed');
    ...
    const task2 = await supabase.rpc('fn_create_putaway_task', {
      p_tenant_id: TENANT,
      p_warehouse_id: warehouseId,
      p_item_id: itemId,
      p_variant_id: null,
    ...
    await supabase
      ...
      .eq('id', task2.data.id);
    await supabase.rpc('fn_assign_warehouse_task', {
      p_task_id: task2.data.id,
      p_tenant_id: TENANT,
      p_assigned_to: null,
      p_actor_id: null,
    });
    const badConfirm = await supabase.rpc('fn_confirm_warehouse_task', {
      p_task_id: task2.data.id,
      p_tenant_id: TENANT,
      p_quantity: 1,
      p_confirmed_location_id: locReceiving,
    ...
    expect(badConfirm.error?.message).toMatch(
      /replenishment task .* has no source_location_id/,
    );
    await supabase.rpc('fn_cancel_warehouse_task', {
      p_task_id: task2.data.id,
      p_tenant_id: TENANT,
      p_actor_id: null,
    });
  ...
  it('9. task assignment and completion lifecycle (pending -> assigned -> completed)', async () => {
    ...
    expect(task.data.status).toBe('pending');
    const assigned = await supabase.rpc('fn_assign_warehouse_task', {
      p_task_id: task.data.id,
      p_tenant_id: TENANT,
      p_assigned_to: null,
      p_actor_id: null,
    ...
    const completed = await supabase.rpc('fn_confirm_warehouse_task', {
      p_task_id: task.data.id,
      p_tenant_id: TENANT,
      p_quantity: 1,
      p_confirmed_location_id: locStorage,
    ...
    expect(history.data.map((h: any) => h.new_status)).toEqual([
      'pending',
      'assigned',
      'completed',
    ]);
  ...
  it('10. picking batch validation: fn_validate_pick_requirements blocks a batch-tracked item with no batch given', async () => {
    const r = await supabase.rpc('fn_validate_pick_requirements', {
      p_tenant_id: TENANT,
      p_warehouse_id: warehouseId,
  ...
  it('12. quality hold blocks picking: reservation respects quantity_quality_held (integration with migration 163)', async () => {
    const balBefore = await supabase
      .from('v_stock_balance')
      .select('quantity_available')
      .eq('tenant_id', TENANT)
      .eq('item_id', itemId)
      .eq('location_id', locReceiving)
      .single();
    const holdQty = Number(balBefore.data.quantity_available); // hold everything currently there

    const holdMv = await supabase.rpc('fn_apply_stock_movement', {
      p_tenant_id: TENANT,
      p_warehouse_id: warehouseId,
      p_location_id: locReceiving,
      p_item_id: itemId,
    ...
    const res = await supabase.rpc('fn_create_reservation', {
      p_tenant_id: TENANT,
      p_warehouse_id: warehouseId,
      p_item_id: itemId,
      p_variant_id: null,
    ...
    for (const h of holds.data ?? []) {
      await supabase.rpc('fn_release_quality_hold', {
        p_hold_id: h.id,
        p_tenant_id: TENANT,
        p_actor_id: null,
  ...
  it('14. permission checks: warehouse.manage and warehouse.approve exist and are seeded', async () => {
    const { data: perms } = await supabase
      .from('permissions')
      .select('name')
      .in('name', ['warehouse.manage', 'warehouse.approve']);
