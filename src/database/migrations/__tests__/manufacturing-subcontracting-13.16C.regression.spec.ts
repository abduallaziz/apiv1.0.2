...
const fakeCache = { get: async () => null, set: async () => undefined, delByPrefix: async () => undefined } as any;
...
describe('manufacturing subcontracting regression (Migration 13.16C)', () => {
  ...
  const seedStock = async (itemId: string, qty: number, unitCost: number) => {
    const { data: gr, error: grErr } = await supabase
      .from('goods_receipts')
      .insert({ tenant_id: TEST_TENANT_ID, warehouse_id: warehouseId, receipt_number: `MFG1316C-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, status: 'draft' })
      .select()
      .single();
  ...
  beforeAll(async () => {
    ...
    subcontractOrdersService = new SubcontractOrdersService(subcontractOrdersRepo, suppliersService, warehousesService, itemsService);
  }, 30_000);

  afterAll(async () => {
    for (const id of orderIds) {
      await supabase.from('subcontract_costs').delete().eq('subcontract_order_id', id);
      await supabase.from('subcontract_order_lines').delete().eq('subcontract_order_id', id);
    }
  ...
  it('Test 2: a supplier can be marked as a subcontractor, existing suppliers unaffected', async () => {
    const plain: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Plain Supplier' } as any);
    supplierIds.push(plain.id);
    expect(plain.is_subcontractor).toBe(false); // default preserved

    const subcontractor: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Subcontractor', is_subcontractor: true } as any);
  ...
  it('Test 1: creates a subcontract order with lines, rejecting a non-subcontractor supplier', async () => {
    const plainSupplier: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Plain For Reject' } as any);
    ...
    await expect(
      subcontractOrdersService.create(TEST_TENANT_ID, null, {
        ...
        lines: [{ material_item_id: material, quantity_sent: 5, output_item_id: output, output_quantity: 4 }],
      } as any),
    ).rejects.toThrow('not marked as a subcontractor');

    const subcontractor: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Sub For Create', is_subcontractor: true } as any);
    supplierIds.push(subcontractor.id);

    const order: any = await subcontractOrdersService.create(TEST_TENANT_ID, null, {
  ...
  it('Test 3, 4: sending materials externally posts a subcontract_out movement and decreases stock', async () => {
    const subcontractor: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Sub Send', is_subcontractor: true } as any);
    supplierIds.push(subcontractor.id);
    ...
    const order: any = await subcontractOrdersService.create(TEST_TENANT_ID, null, {
      supplier_id: subcontractor.id, warehouse_id: warehouseId, order_number: `MFG1316C-SEND-${Date.now()}`,
      lines: [{ material_item_id: material, quantity_sent: 10, output_item_id: output, output_quantity: 8 }],
    } as any);
    orderIds.push(order.id);

    const before = await supabase.from('stock_levels').select('quantity_on_hand').eq('tenant_id', TEST_TENANT_ID).eq('item_id', material).eq('warehouse_id', warehouseId).is('location_id', null).single();

    const sent: any = await subcontractOrdersService.send(order.id, TEST_TENANT_ID, null);
    ...
    const after = await supabase.from('stock_levels').select('quantity_on_hand').eq('tenant_id', TEST_TENANT_ID).eq('item_id', material).eq('warehouse_id', warehouseId).is('location_id', null).single();
    expect(Number(after.data!.quantity_on_hand)).toBe(Number(before.data!.quantity_on_hand) - 10);
  }, 30_000);

  it('Test 5, 6, 7, 8: receiving with a service cost posts subcontract_in with material+service cost included', async () => {
    const subcontractor: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Sub Receive', is_subcontractor: true } as any);
    supplierIds.push(subcontractor.id);
    ...
    const order: any = await subcontractOrdersService.create(TEST_TENANT_ID, null, {
      supplier_id: subcontractor.id, warehouse_id: warehouseId, order_number: `MFG1316C-RECV-${Date.now()}`,
      lines: [{ material_item_id: material, quantity_sent: 10, output_item_id: output, output_quantity: 8 }],
    } as any);
    orderIds.push(order.id);
    ...
    await subcontractOrdersService.addCost(order.id, TEST_TENANT_ID, null, { cost_type: 'service_fee', amount: 40 } as any);
    const costs = await subcontractOrdersService.findCosts(order.id, TEST_TENANT_ID);
    expect(costs.length).toBe(1);

    const received: any = await subcontractOrdersService.receive(order.id, TEST_TENANT_ID, null);
    ...
    const { data: layers } = await supabase.from('cost_layers').select('quantity_remaining, unit_cost').eq('tenant_id', TEST_TENANT_ID).eq('item_id', output);
    ...
    await expect(
      subcontractOrdersService.addCost(order.id, TEST_TENANT_ID, null, { cost_type: 'other', amount: 5 } as any),
  ...
  it('Test 9: a subcontract order can optionally link to a production order, without affecting its own completion', async () => {
    const subcontractor: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Sub PO Link', is_subcontractor: true } as any);
    supplierIds.push(subcontractor.id);
    ...
    const { data: bom, error: bomErr } = await supabase.from('bill_of_materials').insert({ tenant_id: TEST_TENANT_ID, item_id: finished, is_active: true }).select().single();
    if (bomErr) throw bomErr;
    bomIds.push(bom.id);
    await supabase.from('bom_lines').insert({ tenant_id: TEST_TENANT_ID, bom_id: bom.id, component_item_id: component, quantity_per_unit: 1, scrap_percentage: 0 });

    const { data: prodOrder, error: poErr } = await supabase
      .from('production_orders')
      .insert({ tenant_id: TEST_TENANT_ID, warehouse_id: warehouseId, bom_id: bom.id, order_number: `MFG1316C-PO-${Date.now()}`, quantity_planned: 5, status: 'in_progress', started_at: new Date().toISOString() })
      .select()
      .single();
    ...
    const order: any = await subcontractOrdersService.create(TEST_TENANT_ID, null, {
      supplier_id: subcontractor.id, warehouse_id: warehouseId, production_order_id: prodOrder.id,
      order_number: `MFG1316C-LINKED-${Date.now()}`,
      lines: [{ material_item_id: material, quantity_sent: 2, output_item_id: output, output_quantity: 2 }],
    } as any);
    orderIds.push(order.id);
    ...
    const { data: completed, error: completeErr } = await supabase.rpc('fn_post_production_order', {
      p_production_order_id: prodOrder.id, p_actor_id: null,
    });
    expect(completeErr).toBeNull();
    expect(completed.status).toBe('completed');

    const { data: subOrderAfter } = await supabase.from('subcontract_orders').select('status').eq('id', order.id).single();
  ...
  it('Test 10: tenant isolation — subcontract orders and lines are not visible cross-tenant', async () => {
    const subcontractor: any = await suppliersService.create(TEST_TENANT_ID, { name: 'Regr 13.16C Sub Isolation', is_subcontractor: true } as any);
    supplierIds.push(subcontractor.id);
    ...
    const order: any = await subcontractOrdersService.create(TEST_TENANT_ID, null, {
      supplier_id: subcontractor.id, warehouse_id: warehouseId, order_number: `MFG1316C-ISO-${Date.now()}`,
      lines: [{ material_item_id: material, quantity_sent: 1, output_item_id: output, output_quantity: 1 }],
    } as any);
    orderIds.push(order.id);

    await expect(subcontractOrdersService.findById(order.id, OTHER_TENANT_ID)).rejects.toThrow('Subcontract order not found');
    const crossTenantList = await subcontractOrdersService.findAll(OTHER_TENANT_ID);
