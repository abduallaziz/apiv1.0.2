...
describe('MRP Engine (migrations 160-162)', () => {
  ...
  beforeAll(async () => {
    ...
    const mk = async (name: string, type: string) => {
      const { data, error } = await supabase
        .from('items')
        .insert({ tenant_id: TEST_TENANT_ID, name, type, operation_type: 'sell', price: 5, is_active: true })
        .select()
        .single();
    ...
    const { data: subBom } = await supabase
      ...
      .insert({ tenant_id: TEST_TENANT_ID, item_id: subAssemblyId, is_active: true })
      .select()
      .single();
    subAssemblyBomId = subBom.id;
    await supabase.from('bom_lines').insert({
      tenant_id: TEST_TENANT_ID, bom_id: subAssemblyBomId, component_item_id: rawMaterialId, quantity_per_unit: 3,
    });
    ...
    await supabase.from('bom_lines').insert({
      tenant_id: TEST_TENANT_ID, bom_id: finishedBomId, component_item_id: subAssemblyId, quantity_per_unit: 2,
    });
    ...
    const { data: rp } = await supabase
      ...
      .insert({
        tenant_id: TEST_TENANT_ID, warehouse_id: warehouseId, item_id: finishedItemId,
        min_quantity: 20, reorder_quantity: 20, lead_time_days: 5, is_active: true,
      })
      .select()
      .single();
  ...
  afterAll(async () => {
    ...
    await supabase.from('production_orders').delete().eq('tenant_id', TEST_TENANT_ID).in('bom_id', [subAssemblyBomId, finishedBomId]);
    await supabase.from('purchase_request_items').delete().eq('item_id', rawMaterialId);
    await supabase.from('purchase_requests').delete().eq('tenant_id', TEST_TENANT_ID).ilike('notes', '%MRP planned order%');
    await supabase.from('planned_orders').delete().in('item_id', [rawMaterialId, subAssemblyId, finishedItemId]);
    await supabase.from('inventory_reorder_points').delete().eq('id', reorderPointId);
    await supabase.from('bill_of_materials').delete().in('id', [subAssemblyBomId, finishedBomId]);
    await supabase.from('items').delete().in('id', [rawMaterialId, subAssemblyId, finishedItemId]);
  }, 30_000);
  ...
  it('1. single-level BOM explosion: fn_explode_bom_requirements creates a purchase planned order for a leaf raw material', async () => {
    ...
    await supabase.from('planned_orders').delete().eq('mrp_run_id', '11111111-1111-1111-1111-111111111111');
  });
  ...
  it('3. planned purchase order converts into a real purchase_requests row', async () => {
    const { data: runId } = await supabase.rpc('fn_run_mrp', { p_tenant_id: TEST_TENANT_ID, p_warehouse_id: warehouseId });
    const { data: rawRow } = await supabase.from('planned_orders').select('*').eq('mrp_run_id', runId).eq('item_id', rawMaterialId).single();
    expect(rawRow.order_type).toBe('purchase');

    await supabase.from('planned_orders').update({ status: 'approved' }).eq('id', rawRow.id);
    ...
    const { data: pr } = await supabase
      ...
      .insert({ tenant_id: TEST_TENANT_ID, warehouse_id: warehouseId, request_number: `MRP-REGR-PR-${Date.now()}`, status: 'draft', notes: 'Generated from MRP planned order' })
      .select()
      .single();
    await supabase.from('purchase_request_items').insert({ tenant_id: TEST_TENANT_ID, purchase_request_id: pr.id, item_id: rawMaterialId, quantity_requested: rawRow.quantity });
    await supabase.from('planned_orders').update({ status: 'converted', converted_reference_type: 'purchase_request', converted_reference_id: pr.id }).eq('id', rawRow.id);

    const { data: updated } = await supabase.from('planned_orders').select('*').eq('id', rawRow.id).single();
  ...
  it('4. inventory offsets requirements: adding on-hand stock for the raw material reduces its planned quantity on the next run', async () => {
    ...
    const { error: mvError } = await supabase.rpc('fn_apply_stock_movement', {
      p_tenant_id: TEST_TENANT_ID, p_warehouse_id: warehouseId, p_location_id: null,
      p_item_id: rawMaterialId, p_variant_id: null, p_batch_id: null,
      p_movement_type: 'receipt', p_direction: 'in', p_quantity: 1000, p_unit_cost: 1,
      p_reference_type: 'regression_test', p_reference_id: null, p_created_by: null, p_allow_negative: false,
    });
    expect(mvError).toBeNull();

    const { data: runId } = await supabase.rpc('fn_run_mrp', { p_tenant_id: TEST_TENANT_ID, p_warehouse_id: warehouseId });
    const { data: rows } = await supabase.from('planned_orders').select('*').eq('mrp_run_id', runId).eq('item_id', rawMaterialId);
  ...
  it('5. open production order offsets requirements: an in-progress production order for the sub-assembly reduces its planned quantity', async () => {
    const { data: po, error } = await supabase
      ...
      .insert({
        tenant_id: TEST_TENANT_ID, warehouse_id: warehouseId, bom_id: subAssemblyBomId,
        order_number: `MRP-REGR-PO-${Date.now()}`, quantity_planned: 999999, status: 'in_progress',
      })
      .select()
      .single();
    expect(error).toBeNull();
    ...
    const { data: runId } = await supabase.rpc('fn_run_mrp', { p_tenant_id: TEST_TENANT_ID, p_warehouse_id: warehouseId });
    const { data: rows } = await supabase.from('planned_orders').select('*').eq('mrp_run_id', runId).eq('item_id', subAssemblyId);
  ...
  it('6. rerun replaces only proposed planned orders, preserving approved/converted/cancelled history', async () => {
    const { data: runId1 } = await supabase.rpc('fn_run_mrp', { p_tenant_id: TEST_TENANT_ID, p_warehouse_id: warehouseId });
    const { data: firstRun } = await supabase.from('planned_orders').select('*').eq('mrp_run_id', runId1).eq('item_id', finishedItemId).single();

    await supabase.from('planned_orders').update({ status: 'approved' }).eq('id', firstRun.id);

    const { data: runId2 } = await supabase.rpc('fn_run_mrp', { p_tenant_id: TEST_TENANT_ID, p_warehouse_id: warehouseId });
    expect(runId2).not.toBe(runId1);

    // The approved row from run 1 must still exist, untouched.
    const { data: stillThere } = await supabase.from('planned_orders').select('*').eq('id', firstRun.id).single();
    ...
    const { data: secondRun } = await supabase.from('planned_orders').select('*').eq('mrp_run_id', runId2).eq('item_id', finishedItemId);
  ...
  it('7. tenant isolation: fn_run_mrp for a different tenant does not see this tenant\'s reorder point', async () => {
    ...
    expect(error).toBeNull();
    const { data: rows } = await supabase.from('planned_orders').select('*').eq('mrp_run_id', runId);
    expect(rows!.length).toBe(0);
  });
  ...
  it('8. BOM explosion depth guard: a data cycle raises an exception instead of looping forever', async () => {
    ...
    const { data: itemA } = await supabase.from('items').insert({ tenant_id: TEST_TENANT_ID, name: 'MRP Regression Cycle A', type: 'semi_finished', operation_type: 'sell', price: 5, is_active: true }).select().single();
    const { data: itemB } = await supabase.from('items').insert({ tenant_id: TEST_TENANT_ID, name: 'MRP Regression Cycle B', type: 'semi_finished', operation_type: 'sell', price: 5, is_active: true }).select().single();

    const { data: bomA } = await supabase.from('bill_of_materials').insert({ tenant_id: TEST_TENANT_ID, item_id: itemA.id, is_active: true }).select().single();
    const { data: bomB } = await supabase.from('bill_of_materials').insert({ tenant_id: TEST_TENANT_ID, item_id: itemB.id, is_active: true }).select().single();
    await supabase.from('bom_lines').insert({ tenant_id: TEST_TENANT_ID, bom_id: bomA.id, component_item_id: itemB.id, quantity_per_unit: 1 });
    await supabase.from('bom_lines').insert({ tenant_id: TEST_TENANT_ID, bom_id: bomB.id, component_item_id: itemA.id, quantity_per_unit: 1 });

    const { error } = await supabase.rpc('fn_explode_bom_requirements', {
      p_tenant_id: TEST_TENANT_ID, p_warehouse_id: warehouseId, p_bom_id: bomA.id, p_quantity: 1,
      p_mrp_run_id: '22222222-2222-2222-2222-222222222222', p_parent_planned_order_id: null, p_depth: 1,
    });
    expect(error).not.toBeNull();
    expect(error!.message).toMatch(/exceeded max depth/);

    await supabase.from('planned_orders').delete().eq('mrp_run_id', '22222222-2222-2222-2222-222222222222');
    await supabase.from('bill_of_materials').delete().in('id', [bomA.id, bomB.id]);
