...
function fakeConfig(threshold: number) {
  return {
    get: (key: string) =>
      key === 'INVENTORY_COUNT_APPROVAL_THRESHOLD'
        ? String(threshold)
        : undefined,
  } as any;
}
...
describe('stock count completion regression (Migration 13.12-fix)', () => {
  ...
  it('Test 1: full count (default, no count_type) behaves exactly as before — includes all seeded items', async () => {
    ...
    const created: any = await service.create(
      ...
      {
        warehouse_id: warehouseId,
        count_number: `R1312-FULL-${Date.now()}`,
      },
      null,
    );
  ...
  it('Test 2: partial count only includes the selected item scope', async () => {
    ...
    const created: any = await service.create(
      ...
      {
        warehouse_id: warehouseId,
        count_number: `R1312-PARTIAL-${Date.now()}`,
        count_type: 'partial',
        item_ids: [itemA],
      } as any,
      null,
  ...
  it('Test 3: cycle count works, scoped by location', async () => {
    ...
    const created: any = await service.create(
      ...
      {
        warehouse_id: warehouseId,
        count_number: `R1312-CYCLE-${Date.now()}`,
        count_type: 'cycle',
  ...
  it('Test 3b: partial/cycle without any scope is rejected', () => {
    ...
    expect(() =>
      service.create(
        ...
        {
          warehouse_id: warehouseId,
          count_number: `R1312-NOSCOPE-${Date.now()}`,
          count_type: 'partial',
        } as any,
        null as any,
      ),
  ...
  it('Test 4: variance reason code is stored on the count item, and an invalid reason code is rejected', async () => {
    ...
    const created: any = await service.create(
      ...
      {
        warehouse_id: warehouseId,
        count_number: `R1312-REASON-${Date.now()}`,
        count_type: 'partial',
        item_ids: [itemA],
      } as any,
      null,
    ...
    const updated: any = await service.submitCount(
      created.id,
      line.id,
      TEST_TENANT_ID,
      { counted_quantity: 8, reason_code_id: reasonCodeId },
    );
    expect(updated.reason_code_id).toBe(reasonCodeId);

    await expect(
      service.submitCount(created.id, line.id, TEST_TENANT_ID, {
        counted_quantity: 8,
        reason_code_id: '00000000-0000-0000-0000-000000000099',
      } as any),
  ...
  it('Test 5+6+7: approval-required count enters pending_approval, cannot bypass approval, and can finalize once approved', async () => {
    ...
    const created: any = await service.create(
      ...
      {
        warehouse_id: warehouseId,
        count_number: `R1312-APPROVAL-${Date.now()}`,
        count_type: 'partial',
        item_ids: [itemA],
      } as any,
      null,
    ...
    countIds.push(created.id);
    const line = created.items[0];
    await service.submitCount(created.id, line.id, TEST_TENANT_ID, {
      counted_quantity: 5,
    }); // expected 10 -> variance 5 * 50 = 250

    // Test 5: finalize is blocked and flips the count to pending_approval
    await expect(
      service.finalize(created.id, TEST_TENANT_ID, null as any),
    ).rejects.toThrow('requires approval before finalizing');
    const afterFirstAttempt: any = await service.findById(
      created.id,
      TEST_TENANT_ID,
    );
    ...
    const { error: directRpcError } = await supabase.rpc(
    ...
    expect(directRpcError?.message).toContain(
      'requires approval before finalizing',
    );

    // Test 6: once approved, finalize succeeds
    await service.approve(created.id, TEST_TENANT_ID, null, { approved: true });
    const finalized: any = await service.finalize(
      created.id,
      TEST_TENANT_ID,
      null,
    );
  ...
  it('Test 8: existing finalize flow without an approval threshold remains unchanged (threshold=0)', async () => {
    ...
    const created: any = await service.create(
      ...
      {
        warehouse_id: warehouseId,
        count_number: `R1312-NOTHRESH-${Date.now()}`,
        count_type: 'partial',
        item_ids: [itemA],
      } as any,
      null,
    ...
    await service.submitCount(created.id, line.id, TEST_TENANT_ID, {
      counted_quantity: 5,
    }); // large variance, but threshold disabled
  ...
  it('Test 9: tenant isolation — a count/reason-code from another tenant is not accessible', async () => {
    ...
    const created: any = await service.create(
      ...
      {
        warehouse_id: warehouseId,
        count_number: `R1312-ISO-${Date.now()}`,
        count_type: 'partial',
        item_ids: [itemA],
      } as any,
      null,
    ...
    await expect(service.findById(created.id, OTHER_TENANT_ID)).rejects.toThrow(
      'Stock count not found',
    );

    const exists = await countsRepo.reasonCodeExists(
      reasonCodeId,
      OTHER_TENANT_ID,
    );
