...
const permissions = [
  ...
  { key: 'analytics.view.all', resource: 'analytics', action: 'view', scope: 'all', description: 'View platform analytics' },

  // Audit (superadmin only)
  { key: 'audit.view.all', resource: 'audit', action: 'view', scope: 'all', description: 'View all audit logs' },

  // Queue Management (superadmin only)
  { key: 'superadmin.queue.view', resource: 'superadmin', action: 'queue.view', scope: 'all', description: 'View job queues and stats' },
  { key: 'superadmin.queue.manage', resource: 'superadmin', action: 'queue.manage', scope: 'all', description: 'Pause, resume, retry, clean queues' },

  // Health (superadmin only)
  { key: 'superadmin.health.view', resource: 'superadmin', action: 'health.view', scope: 'all', description: 'View system health dashboard' },

  // Backup (superadmin only)
  { key: 'superadmin.backup.view', resource: 'superadmin', action: 'backup.view', scope: 'all', description: 'View backup and disaster recovery status' },
];
...
const superadminPermissions = [
  ...
  'audit.view.all',
  'invoice.view.all',
  'expense.view.all', 'expense.approve', 'expense.reject',
  'shift.view.all',
  'users.manage', 'branches.manage', 'items.manage',
  'reports.view.all', 'settings.manage',
  'superadmin.queue.view', 'superadmin.queue.manage',
...
addRolePermissions('superadmin', superadminPermissions);
