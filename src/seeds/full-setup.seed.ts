...
async function seed() {
  ...
  const ownerPassword =
    process.env.SEED_OWNER_PASSWORD ?? crypto.randomBytes(12).toString('hex');
  ...
  if (rpError) {
    console.error('❌ role_permissions:', rpError.message);
    process.exit(1);
  }
  ...
  const { error: saError } = await supabase.from('users').upsert(
    {
  ...
  if (saError) {
    console.error('❌ superadmin:', saError.message);
    process.exit(1);
  }
  ...
  if (featError) {
    console.error('❌ features:', featError.message);
    process.exit(1);
  }
  ...
  if (pfError) {
    console.error('❌ plan_features:', pfError.message);
    process.exit(1);
  }
  ...
  const { data: tenant, error: tenantError } = await supabase
    ...
    .single();
  if (tenantError) {
    console.error('❌ tenant:', tenantError.message);
    process.exit(1);
  }
  ...
  if (subError) {
    console.error('❌ subscription:', subError.message);
    process.exit(1);
  }
  ...
  if (ownerError) {
    console.error('❌ owner:', ownerError.message);
    process.exit(1);
  }
  ...
  if (branchError) {
    console.error('❌ branch:', branchError.message);
    process.exit(1);
  }
